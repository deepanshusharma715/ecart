...
def homepage(Request):
    ...
    ldata=[]    
    for item in Brands:
        ldata.append(item.name)
    Request.session['Brands']=ldata

    ldata=[]
    for item in maincategories:
        ldata.append(item.name)
        Request.session['maincategories']=ldata

    ldata=[]
    for item in subcategories:
        ldata.append(item.name)
    Request.session['subcategories']=ldata
...
def shoppage(Request,mc,sc,br):
    if(mc=="All" and sc=="All" and br=="All"):
        ...
        data=product.objects.filter(subcategory=subcategory.objects.get(name=sc),brands=Brand.objects.get(name=br)).order_by("-id")
    else:
...
def filterpage(Request,mc,sc,br,filter):
    if(filter=="Latest"):
        ...
        if(mc=="All" and sc=="All" and br=="All"):
            ...
            data=product.objects.filter(subcategory=subcategory.objects.get(name=sc),brands=Brand.objects.get(name=br)).order_by("finalprice")
        else:
...
def priceFilterpage(Request,mc,sc,br):
    ...
    if(mc=="All" and sc=="All" and br=="All"):
        ...
        data=product.objects.filter(subcategory=subcategory.objects.get(name=sc),brands=Brand.objects.get(name=br),finalprice__gte=min,finalprice__lte=max).order_by("-id")
    else:
        data=product.objects.filter(maincategory=maincategory.objects.get(name=mc),subcategory=subcategory.objects.get(name=sc),Brand=Brand.objects.get(name=br),finalprice__lte=max).order_by("-id")
...
def singleproductpage(Request,id):
    ...
    relatedproduct=product.objects.filter(maincategory=maincategory.objects.get(name=data.maincategory))
    return render(Request,"detail.html",{'data':data,'relatedproduct':relatedproduct})
...
def contactPage(Request):
    if(Request.method=="POST"):
        ...
        c.save()
        messages.success(Request,"Thank to Share Your Query With us !!! Our team will Contact soon!!!! ")
         ...
         if(Request.POST.get("password")!=Request.POST.get("cpassword")):
            ...
            try:
                ...
                return HttpResponseRedirect("/login/")
                 ...
                 messages.error(Request,"Already user taken!!")        
...
def profilePage(Request):
    ...
    if(user.is_superuser):
        ...
        for item in checkout:
            cp=CheckoutProduct.objects.filter(checkout=item.id)
            order.append({'checkout':item,'checkoutProduct':cp})
        return render(Request,"profile.html",{'data':buyer,'wishlist':wishlist,'order':order})
...
def forgetPasswordPage1(Request):
    if(Request.method=="POST"):
        ...
        try:
            ...
            subject = 'OTP for Password Reset:Team ECART'
            message = """Hello"""+username+"""
            OTP For Password Reset is""" + str(otp)+ """
            Never Share OTP With Anyone Team:Ecart"""
            """  
            """
            email_from = settings.EMAIL_HOST_USER
